...
const valueTypes = [
  { key: 'liquidatiewaarde', labelKey: 'liquidationValue' },
  { key: 'marktwaarde', labelKey: 'marketValue' },
  { key: 'onderhandseVerkoopwaarde', labelKey: 'privateValue' },
  { key: 'vervangingswaarde', labelKey: 'replacementValue' },
]
...
export default function TaxateurAdjust({ prices, onComplete }) {
  ...
  const [reason, setReason] = useState('noAdjustment')
  ...
  return (
    <div className="space-y-6">
            ...
            return (
              <div key={vt.key} className="p-4 bg-ntab-light rounded-lg">
                <div className="flex items-center justify-between mb-2">
                  <label className="text-sm font-medium text-ntab-text">{t(vt.labelKey)}</label>
                  <span className="text-xs text-ntab-text-light">
                    {t('suggestion')}: {formatCurrency(suggested)}
                  </span>
                </div>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-ntab-border p-6">
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-ntab-text mb-1">{t('adjustmentReason')}</label>
          <select
            value={reason}
        ...
        <div className="mb-4">
          <label className="block text-sm font-medium text-ntab-text mb-1">{t('taxateurNotes')}</label>
          <textarea
            value={notes}
            onChange={e => setNotes(e.target.value)}
            rows={4}
            placeholder={t('notesMotivation')}
        ...
        <div>
          <label className="block text-sm font-medium text-ntab-text mb-1">
            {t('reliabilityTaxation')}: <span className="font-bold">{confidence}%</span>
          </label>
          <input
            type="range"
            min="10"
